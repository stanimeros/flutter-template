...
class FirestoreService {
  ...
  Future<List<AppUser>> getFriends() async {
    try {
      ...
      // Fetch AppUser documents for each unique friend ID
      List<AppUser> friends = [];
      for (String friendId in friendIds) {
        DocumentSnapshot friendDoc = await firestore.collection('users').doc(friendId).get();
        if (friendDoc.exists) {
          friends.add(AppUser.fromFirestore(friendDoc));
        }
      }
  ...
  Future<List<AppUser>> getUsersRequested() async {
    try {
      ...
      List<Future<AppUser?>> userFutures = docs.map((doc) {
        return FirestoreService().getUser(doc['uid']);
      }).toList();
      
      // Wait for all futures to complete
      List<AppUser?> users = await Future.wait(userFutures);
      List<AppUser> nonNullUsers = users.where((user) => user != null).cast<AppUser>().toList();
